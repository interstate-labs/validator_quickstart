# Quickstart
1. Run eth-docker
    - Follow the instructions here: https://github.com/eth-educators/eth-docker
2. run docker ps -a
    - this will display the port numbers, which are the inputs in the next step
3. Copy env variables into bolt docker image
    - Generate a JWT for the engine API: openssl rand -hex 32 | tr -d "\n" > jwtsecret
    - find BLS Key for your validators
    - Find the env variables and add to the launch.env file
4. Run bolt docker image
...
- Interstate relay info: http://helder-relay.interstate.chainbound.io:9060

## Components

The components that need to run to test Interstate on a devnet are:

- Execution client
- Beacon node
- Active validators
- **Interstate sidecar**
- **Interstate mev-boost**
- **Interstate relay**
- **Interstate builder**

We will run an instance of the Interstate relay and Interstate builder, and can run the Interstate sidecar and modified mev-boost if required. Schematic: 
...
> [!IMPORTANT]  
> The beacon node’s `builder-api` should point to the Interstate sidecar API to activate Interstate! It MUST also have
...
lighthouse beacon_node \
    ...
    --builder=localhost:18551 \ # This will be the Interstate builder API
...
## Interstate Sidecar + Interstate MEV-Boost

Sidecar code: https://github.com/chainbound/interstate/tree/unstable/interstate-sidecar

Modified mev-boost code: https://github.com/chainbound/interstate/tree/unstable/mev-boost

We provide a Docker compose script to run these 2 together in the root directory. First, update the `launch.env` file with your correct values. The relay is set to our Interstate-enabled relay.
...
# Interstate-enabled relay on Helder
INTERSTATE_RELAY="http://0xa55c1285d84ba83a5ad26420cd5ad3091e49c55a813eee651cd467db38a8c8e63192f47955e9376f6b42f6d190571cb5@135.181.191.125:9062"
...
> [!IMPORTANT]  
> The Interstate RPC port must be exposed to the internet to allow users to send commitment requests to it.
> The RPC will be registered in an on-chain registry in the next step.

## Validators

In this section we cover how one should configure their validators and register them in the Interstate on-chain [registry](https://github.com/chainbound/interstate/tree/unstable/interstate-contracts#registry).
...
The `--builder-proposals` and `--prefer-builder-proposals` flags are required so that Lighthouse won’t rely on EL self-building, which in some cases could lead to commitment faults. The Interstate sidecar deals with PBS failures gracefully and will build its own local block in case the relay / builder goes down.

### Interstate Registration

Next up, we need to register the validators in the Interstate smart contract registry, for which we provide some Forge scripts:
...
- Clone the Interstate repo and navigate to the [contracts](https://github.com/chainbound/interstate/tree/unstable/interstate-contracts) directory:

```bash
git clone https://github.com/chainbound/interstate
cd interstate-contracts
forge install
```
...
```bash
# The ECDSA private key of your Interstate operator
export PRIVATE_KEY="0x..."
# The Interstate RPC address (public)
